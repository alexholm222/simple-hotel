...
function Validator() {
    ...
    const handleChange = (event) => {
      ...
      setErrors({...errors, [name]: `${target.value ==='' ? 'Заполните поле' : target.checkValidity() ? '' : target.closest('.auth__input_password') ? 'Пароль должен быть длинее 8 символов, без кириллицы' : 'укажите email'}`});
